...
| Systemplane cold-tenant resolution | Either (a) seed migration `000NNN_systemplane_defaults_seed.up.sql` + `make check-systemplane-seed` drift guard wired into `check-generated-artifacts`, OR (b) `Client.BindManager` once on `lib-systemplane v1.5.0+`. NON-COMPLIANT if both are missing in MT. |
...
| **HIGH** | Tenant context missing in background jobs, non-canonical env var name, manual Redis pub/sub client instead of `tmredis.NewTenantPubSubRedisClient`, **adapter package importing `lib-systemplane` directly instead of declaring a narrow DI interface**, **seed migration without matching `make check-systemplane-seed` drift guard** |
