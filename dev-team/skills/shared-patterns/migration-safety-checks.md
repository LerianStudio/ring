...
**Home:** These checks are executed by [`ring:dev-cycle` Step 12.0.5b](../dev-cycle/SKILL.md) — Gate 0.5D, a post-cycle conditional gate that runs once per cycle when SQL migration files are present in the diff against `origin/main`. Gate 0.5D is parallel to Gate 0.5G (multi-tenant dual-mode verification at Step 12.0); 0.5G checks Go code safety while 0.5D checks SQL schema evolution safety.
