...
| ring:multi-tenant-reviewer | If service is multi-tenant, namespace strategy does NOT leak cross-tenant config; systemplane knobs are service-scoped not tenant-scoped (per-tenant config belongs in tenant-manager, not systemplane) |
