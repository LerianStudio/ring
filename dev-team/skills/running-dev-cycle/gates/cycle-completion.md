...
6. **Lerian Map Sync (only when `state.lerian_map_sync.enabled`):**
   - **Epic `done` push:** an epic flips to the terminal `done` column **only when its commit is pushed to the repo (≥ `develop`)** — the push-to-develop / PR-merge event, **NOT Gate 9**. When the Final Commit is pushed to at least `develop`, fire the async board push → absolute column `done` on the EPIC `card_id` (`epic_matches[].status_dispatch`) for the affected epics. The same push stamps the epic card's `endDate` and re-asserts the evidence comments' PR link idempotently (the link was filled at the `to_review` push; comment UPDATE — never a second comment). With `commit_timing == "at_end"`, this push is also the COMMIT moment where each task's evidence comment POSTs on the epic card (tagged `**Task N.M.T:** …`), with full evidence. Per SKILL.md '### Date stamping (start/end)' and '### Evidence & enrichment (comments, repositoryPath)'. (If this cycle does not push to `develop`, the epics remain in `to_review` / `testing` and flip to `done` later via the PR-merge event — dates and enrichment ride that later push the same way.)
