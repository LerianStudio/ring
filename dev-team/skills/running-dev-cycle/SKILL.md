...
**Card creation here is now OPTIONAL / FALLBACK.** The canonical owner of epic-card creation is the planning workflow (`ring:planning-small-features` / `ring:planning-large-features`), which now creates the Lerian Map cards as a MANDATORY final step and persists `featureId`, `devMilestoneId`, and the per-epic `card_id`s into `docs/pre-dev/{feature}/workflow-state.json` (`lerianMap` block). This handshake therefore takes one of two paths:

- **Planning-state path (PRIMARY — cards already exist):** when `docs/pre-dev/{feature}/workflow-state.json` carries a `lerianMap` block with `cardsCreated == true` (i.e. `featureId` + `devMilestoneId` + a non-empty `cards[]`, each with a `cardId`), planning already created the cards. In this case:
  - Map sync is implicitly ENABLED — do NOT ask the user the Lerian Map / Task-source opt-in for card creation; adopt `featureId`, `devMilestoneId`, and the per-epic `cardId`s (+ `checklistItemIds`) straight from the block. **Consent model (no silent escalation):** accepting planning's Map Card Creation step — itself an explicit, previewed, user-confirmed action — IS the consent to dev-cycle Map sync. No separate opt-out is offered on this path by design: card creation and the dev-cycle push that updates those same cards are one continuous, user-authorized flow. (To run the cycle WITHOUT Map sync, omit/disable the `lerianMap` block at planning time so this path is not taken.)
  - SKIP card creation (handshake steps 2–4 below) — do NOT re-run the create-with-preview flow.
  - VALIDATE only: for each recorded `cardId`, confirm it still exists on the board via `GET /tasks/{id}` (through `ring:delegating-to-gandalf`). A card that still exists is adopted as-is (NEVER recreated), reusing its persisted `checklistItemIds` directly — the `GET /tasks/{id}` existence check already returns the card, so no extra round-trip is incurred and the persisted ids are the source of truth (re-read the fetched checklist only to reconcile if the board drifted). Only a card that no longer exists (deleted on the board) is re-created, via the steps 2–4 path for that single epic, re-resolving its checklist-item ids from the freshly created card.
  - Still run step 0 (acting-user resolution) and step 5 (persist) so attribution + `state.lerian_map_sync` are populated.
- **Fallback path (NO planning state — backward-compat):** when there is no `lerianMap` block (the feature was planned before this flow, or planning was done manually), or when a `lerianMap` block exists with `cardsCreated: false` (e.g. a Map outage prevented planning from creating the cards), run the FULL handshake (steps 0–5 below) exactly as today, INCLUDING the user's Lerian Map sync opt-in via the cycle-init Task-source question — the card-creation discovery handshake (steps 1–4) is the source of the cards. Behavior is unchanged from today. The user is asked about Map sync ONLY on this path.

When `state.lerian_map_sync.enabled` (set by the planning-state path above, or by the user via the cycle-init Task-source question on the fallback path), run the one-time handshake (each Gandalf ask is a fresh, self-contained session carrying the full discovery payload):
