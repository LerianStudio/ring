...
- **Planning-state path (PRIMARY — cards already exist):** when `docs/pre-dev/{feature}/workflow-state.json` carries a `lerianMap` block with `cardsCreated == true` (i.e. `featureId` + `devMilestoneId` + a non-empty `cards[]`, each with a `cardId`), planning already created the cards. In this case:
  - Map sync is implicitly ENABLED — do NOT ask the user the Lerian Map / Task-source opt-in for card creation; adopt `featureId`, `dev_milestone_id`, and the per-epic `card_id`s (+ checklist item ids) straight from the block.
  - SKIP card creation (handshake steps 2–4 below) — do NOT re-run the create-with-preview flow.
  - VALIDATE only: for each recorded `card_id`, confirm it still exists on the board via `GET /tasks/{id}` (through `ring:delegating-to-gandalf`). A card that still exists is adopted as-is (NEVER recreated); only a card that no longer exists (deleted on the board) is re-created, via the steps 2–4 path for that single epic. Resolve checklist-item ids from the fetched cards.
