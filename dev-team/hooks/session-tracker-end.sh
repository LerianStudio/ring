...
REASON="$(printf '%s' "$INPUT" | jq -r '.matcher // .source // .event // "clear"' 2>/dev/null || printf 'clear')"
SESSION_FILE="$(ring_telemetry_session_file)"

if [[ -f "$SESSION_FILE" ]]; then
  PAYLOAD="$(jq -n --arg reason "$REASON" '{reason:$reason}' 2>/dev/null)"
  EVENT="$(ring_telemetry_envelope_json 'session.end' "$PAYLOAD")"
  ring_telemetry_debug "session.end reason=$REASON"
...
PAYLOAD="$(jq -n --arg trigger "$REASON" '{trigger:$trigger}' 2>/dev/null)"
EVENT="$(ring_telemetry_envelope_json 'session.start' "$PAYLOAD")"
ring_telemetry_debug "session.start fresh_after=$REASON id=$SESSION_ID"
ring_telemetry_post_background "$EVENT"
exit 0
